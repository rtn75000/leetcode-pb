...
on veut avoir  "result is the smallest in lexicographical order" (rappel: 'az' est plus petit que 'ba' dans l'ordre lexical car on regarde la premiere lettre et si la premiere est egale on compare la deuxieme etc...
(c'est comme lordre des mots dans un dictionnaire) ) .
Pour cela si je sais qu'une lettre va se repeter alors si la lettre qui vient apres la lettre qui se repete, est plus petite que cette derniere il vaut mieu que on supprime la lettre qui se repete comme ca elle viendra
apres la lettre qui lui est plus petite est donc avoir un ordre lexicale plus petit. exemple si j'ai 'cbac' quand il faut supprimer le premier 'c' car apres lui y'a 'b' et c se repete ensuite donc vaut mieux avoir 'b...c'
que 'cb...'.
...
l'algo va lire un char de la string puis va l'ajouter au resultat si on lit ensuite un char plus petit et que le char rajouter au resultat ce repete plus tard alors on enleve du resultat la lettre rajouter puis 
on rajoute a la fin la nouvelle lettre 
...
-on lit 'b' : res='ab'
-on lit 'c' : res='abc'
avant d'ajouter une lettre il faut verifier que celle ci ne se trouve pas deja dans le resultat car si elle se trouve dans le resultat ca veut dire que on a fait le choix de la garder car elle nous donne un ordre lexical plus petit ex si on a 'cdc' alors :
-on lit 'c' : res = 'c'
